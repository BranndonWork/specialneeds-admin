...
import type { FilterElementProps, IListing } from "../../interfaces";
...
export const DirectoryList: React.FC = () => {
  const columns = React.useMemo<ColumnDef<IListing>[]>(
    () => [
      {
        id: "title",
        header: "Title",
  ...
  const activeCategorySlug = (currentFilters as any[]).find((f: any) => f.field === "category")?.value as string | undefined;

  const handleCategoryFilter = (slug: string | undefined) => {
    setColumnFilters((prev) => {
      const without = prev.filter((f) => f.id !== "category");
      return slug ? [...without, { id: "category", value: slug }] : without;
    });
  };
