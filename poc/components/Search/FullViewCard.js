...
const SearchResultCardFull = ({
  ...
  if (!type || (typeof type === "undefined" && item?.author)) {
    type = "article";
  }
  ...
  const displayImages = (images, type) => {
    ...
    if (!item.images || item.images.length === 0) {
      if (type === "listings") {
        ...
        url = `/images/missing-${type}-image.jpg`;
        alt = item.title + " featured image";
      }
      item.images = [{ url: url, alt: alt }];
    }
    url = serveAsset(item.images[0].url, cardWidth);
    alt = item.images[0].alt || item.title + " featured image";

    return (
      <div className="listings-image">
        <Image src={url.replace(/^http:\/\//i, "https://")} alt={alt} className="single-image" width={600} height={400} />
        <div
          className="category-name"
          onClick={(e) => {
            e.preventDefault();
            handleCategoryClick(item.category.slug);
          }}
        >
  ...
  const displayDistance = () => {
    if (item.distance) {
      return <span className="distance"> - {item.distance} miles away</span>;
    }
  };
                ...
                {item.address && (
                  <ul className="listings-meta">
                    <li>
                      ...
                      {item.address.state_province} {displayDistance()}
                    </li>
                  </ul>
                )}
                <h3>{item.title}</h3>
...
export default SearchResultCardFull;
