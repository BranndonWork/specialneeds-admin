...
export default function ArticleList() {
  const { tableQuery, current, setCurrent, pageCount } = useTable({ resource: "articles" });
  const articles = tableQuery?.data?.data ?? [];
  const isLoading = tableQuery?.isLoading;

  return (
    <AdminLayout>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        <h2 style={{ margin: 0, fontSize: "18px", color: "#221638", fontWeight: 600 }}>Articles</h2>
        <button style={{
          background: "#0281c4",
          color: "#fff",
          border: "none",
            ...
            <tbody>
              {articles.map((item, i) => (
                <tr key={item.id} style={{ borderBottom: "1px solid #f0f0f0", background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                  ...
                  <td style={{ padding: "10px 16px" }}>
                    <Link href={`/admin/articles/${item.id}`} style={{ color: "#0281c4", fontSize: "12px", marginRight: "12px", textDecoration: "none" }}>Edit</Link>
      ...
      {pageCount > 1 && (
        <div style={{ marginTop: "16px", display: "flex", gap: "8px", justifyContent: "flex-end" }}>
          ...
          <span style={{ padding: "6px 10px", fontSize: "13px", color: "#555" }}>Page {current} of {pageCount}</span>
